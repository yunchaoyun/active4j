...
import com.active4j.dao.func.timer.dao.QuartzJobDao;
...
import com.active4j.service.func.timer.service.QuartzService;
	...
	public QuartzJobModel getJobDetailModel(String id) {
		...
		if(null != job) {
			...
			if(null != job.getPreviousTime()) {
				model.setPrevious(DateUtils.getDate2Str(job.getPreviousTime()));
			}
			//下一次执行时间的处理
			if(null != job.getNextTime()) {
				model.setNext(DateUtils.getDate2Str(job.getNextTime()));
			}
